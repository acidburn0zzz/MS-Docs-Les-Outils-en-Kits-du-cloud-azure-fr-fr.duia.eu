...
ms.locfileid: "83230513"
---
# <a name="get-started-accelerate-migration"></a>Bien démarrer : Accélérer la migration

L’alignement correct des parties prenantes de l’entreprise et des services informatiques peut aider votre organisation à surmonter les obstacles à la migration et à accélérer les efforts de migration. Cet article fournit les étapes recommandées pour l’alignement des parties prenantes, la planification de la migration, le déploiement d’une zone d’atterrissage et la migration des 10 premières charges de travail. Il peut également contribuer à la réussite à long terme due à une gouvernance et à une gestion adéquates.

Utilisez ce guide pour réduire la quantité de matériau et de processus nécessaires à l’alignement d’un effort de migration global. Ce processus utilise les sections du Cloud Adoption Framework qui sont mises en évidence dans l’illustration suivante.

![Bien démarrer avec la migration dans Azure](../_images/get-started/migration-map.png)

Si votre scénario de migration est atypique, vous pouvez obtenir une évaluation personnalisée de la préparation à la migration de votre organisation en utilisant l’[évaluation menée par l’outil de préparation et de migration stratégique (SMART)](https://docs.microsoft.com/assessments/?id=strategic-migration-assessment) pour identifier les conseils les mieux adaptés à vos besoins actuels.

## <a name="get-started"></a>Bien démarrer

L’effort et le processus techniques nécessaires à la migration des charges de travail sont relativement simples. L’achèvement complet du processus de migration est important. Toutefois, la préparation à la migration stratégique aura un impact encore plus important sur le calendrier et la réussite de la migration globale.

L’accélération de l’adoption consiste à effectuer les étapes nécessaires pour seconder l’équipe d’adoption du cloud pendant la migration. Ce guide décrit ces tâches itératives sous forme de liste de contrôle linéaire afin d’aider les clients à appréhender correctement l’itinéraire de migration vers le cloud. Pour illustrer l’importance des étapes de support, la migration figure à l’étape 10 de cet article. En réalité, l’équipe chargée de l’adoption du cloud lancera probablement la première migration pilote en parallèle avec l’étape 4 ou 5.
...
La découverte et l’évaluation offrent un niveau d’alignement technique plus approfondi afin d’élaborer un plan d’action pour la mise en œuvre de la stratégie. Au cours de cette étape, l’analyse de rentabilisation est validée avec des données sur l’état actuel de l’environnement, une analyse quantitative de ces données et une évaluation qualitative approfondie des charges de travail les plus prioritaires.

- [Inventorier les systèmes existants](../digital-estate/inventory.md) : La première étape consiste à comprendre l’état actuel à partir d’une approche programmatique pilotée par les données. Découvrez et rassemblez les données pour activer toutes les activités d’évaluation.
...
Le modèle de plan d’adoption du cloud fournit une approche accélérée pour développer un backlog du projet. Le backlog peut ensuite être modifié pour refléter les résultats de la découverte, la rationalisation, le développement de compétences et la mise en place de partenariats.
...
- Mettre à jour le modèle pour qu’il reflète les 10 premières charges de travail à migrer.
- Mettre à jour les personnes et la vélocité pour estimer le calendrier de mise en production.
- Risques liés au calendrier :
  - Une connaissance insuffisante d’Azure DevOps peut ralentir le processus de déploiement.
  - La complexité et les données disponibles pour chaque charge de travail peuvent également avoir un impact sur le calendrier.
...
Les employés existants peuvent jouer un rôle pratique dans le cadre de l’effort de migration. Toutefois, des compétences supplémentaires peuvent être nécessaires. Lors de cette étape, l’équipe effectuera une auto-évaluation afin d’identifier les opportunités de développement de ces compétences ou utilisera des partenaires pour améliorer ces compétences.

- [Créer un plan de préparation des compétences](../plan/adapt-roles-skills-processes.md). Évaluez rapidement les compétences requises et existantes pour mieux comprendre les exigences en matière de compétences à satisfaire.

**Livrables :**

- Ajouter un plan de préparation des compétences au [modèle de stratégie et de plan](https://archcenter.blob.core.windows.net/cdn/fusion/readiness/Microsoft-Cloud-Adoption-Framework-Strategy-and-Plan-Template.docx).
...
Toutes les ressources migrées sont déployées dans une zone d’atterrissage. Initialement, la zone d’atterrissage sera simple, afin de prendre en charge des charges de travail plus petites. Au fil du temps, elle sera mise à l’échelle afin de gérer des charges de travail plus complexes.
...
- [Développer votre zone d’atterrissage](../ready/considerations/index.md) : Quel que soit le point de départ, identifiez les lacunes dans la zone d’atterrissage déployée afin d’ajouter les composants nécessaires pour l’organisation des ressources, la sécurité, la gouvernance, la conformité, les opérations, et ainsi de suite.

**Livrables :**

- Déployer une première zone d’atterrissage pour les migrations initiales à faible risque.
- Développer un plan pour refactoriser avec le CCoE ou le service Informatique centralisé.
- Risques liés au calendrier :
  - Les exigences de gouvernance, d’exploitation et de sécurité des 10 premières charges de travail peuvent considérablement ralentir ce processus.
  - La refactorisation réelle de la première zone d’atterrissage et des zones d’atterrissage suivantes prendra beaucoup plus de temps, mais devra s’effectuer parallèlement aux efforts de migration.
...
L’effort technique requis pour migrer vos 10 premières charges de travail est relativement simple. Il s’agit également d’un processus itératif que vous répéterez à mesure que vous migrerez davantage de ressources. Le processus consiste à évaluer vos charges de travail (voir l’**étape 4**), à les déployer, puis à les publier dans votre environnement de production.

![Phases de l’effort de migration itérative : évaluer, déployer, publier](../_images/migrate/methodology-effort-only.png)

Les outils de migration cloud permettent de migrer toutes les machines virtuelles d’un centre de communication en une seule passe ou itération. Toutefois, il est plus courant de migrer un plus petit nombre de charges de travail lors de chaque itération. Le fractionnement de la migration en plus petites vagues ou mises en production nécessite davantage de planification, mais réduit les risques techniques et l’impact de la gestion des modifications organisationnelles.

À chaque itération, l’équipe d’adoption du cloud sera plus performante dans la migration des charges de travail. Les étapes suivantes permettront de lancer l’équipe technique sur cette courbe de maturité :

1. Migrez vos **premières charges de travail** en adoptant une approche IaaS pure, à l’aide des outils décrits dans le [guide de migration Azure](../migrate/azure-migration-guide/index.md).
2. Développez les options des outils pour utiliser **la migration et la modernisation** à l’aide des [scénarios de migration](../migrate/azure-best-practices/contoso-migration-overview.md).
...
- Risques liés à la chronologie :   - L’amélioration des stratégies et de l’implémentation de la gouvernance peut ajouter de une à quatre semaines par discipline.
...
Les étapes décrites dans ce guide peuvent aider vos équipes à accélérer leurs efforts de migration grâce à une meilleure gestion des changements et à un alignement des parties prenantes. Ces étapes ralentiront peut-être le processus, mais elles élimineront les blocages courants et accéléreront la réalisation de la valeur commerciale.

## <a name="next-steps"></a>Étapes suivantes

Le Framework d’adoption du cloud est une solution de cycle de vie. Il peut vous aider à commencer un parcours de migration, mais également à faire progresser la maturité des équipes qui prennent en charge les efforts de migration. Les équipes suivantes peuvent utiliser ces prochaines étapes pour que leurs efforts gagnent en maturité. Ces processus parallèles ne sont pas linéaires et ne doivent pas être considérés comme des points de blocage. Au lieu de cela, chacun est un flux de valeur parallèle qui aide à faire murir la préparation globale au cloud de votre entreprise.

| Équipe  | Itération suivante |
|---|---|
| Équipe&nbsp;d’adoption du&nbsp;cloud | [Améliorations des processus](../migrate/migration-considerations/index.md) fournit des informations sur la transition vers une fabrique de migration avec des fonctionnalités de migration continues efficaces. |
